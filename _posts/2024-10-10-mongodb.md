...
- Below we does not specify any network while launching container so bridge network will be assigned by Docker by default

```
docker run -d -p 8003:8501 --name flood-ctnr flood-image:1.0
```
