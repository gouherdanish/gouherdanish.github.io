...
$$ \mathbb{L} = - \sum_{i=1}^n p(x_i) log(p( x_i)) + \lambda (\sum_{i=1}^n p(x_i) - 1) $$

$$ \mathbb{L} = - {p(x_1) log_(p( x_1)) + p(x_2) log_(p( x_1)) + ... + p(x_n) log_(p( x_n))} + \lambda {(p(x_1) + p(x_2) + ... + p(x_n) - 1)} $$

$$ \frac{\partial \mathbb{L}}{\partial p(x_1)} = -(p(x_1)\.{1 \over p(x_1)} + log_(p( x_1)\.1)$$
